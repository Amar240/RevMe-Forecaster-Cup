...
export async function POST(request: Request) {
  try {
    ...
    return jsonOk(await suggestRosterRepairs({ actor: user!, seasonId, batchId: data.batchId, fileHash: data.fileHash, fileName: data.fileName, fileBuffer: data.fileBuffer, columnMapping: data.columnMapping, overrides: data.overrides }))
