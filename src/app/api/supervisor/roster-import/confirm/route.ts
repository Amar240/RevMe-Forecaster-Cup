...
export async function POST(request: Request) {
  try {
    ...
    const { fileName, fileBuffer, batchId, fileHash, overrides, columnMapping, excludedRowNumbers } = await readTeamImportFormData(new Request(request.url, { method: 'POST', body: form }))
    return jsonOk(await confirmRosterImport({ actor: user!, mode: 'supervisor', seasonId: season.id, batchId, fileName, fileBuffer, submittedFileHash: fileHash, overrides, columnMapping, excludedRowNumbers }))
